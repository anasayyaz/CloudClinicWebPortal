...
export default function Device() {
    ...
    } = useFetch(`${BASE_URL}api/device?pageNumber=1&pageSize=100&QuerySearch=`);

    const filteredDevices = deviceList?.filter((device) => device?.name?.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    const handleSubmit = async (values, resetForm) => {
        ...
        try {
            ...
            if (res?.data) {
                refetchDeviceList();
                resetForm();
                return toast.success('Device Added Successfully');
            }
        } catch (error) {
    ...
    const handleUpdate = async (values, resetForm) => {
        ...
        try {
            ...
            const res = await axios({
                method: 'put',
                data,
                url: `${BASE_URL}api/device/${selectedRowToUpdate?.deviceID}`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${user?.token}`
                }
            ...
            if (res?.data) {
                ...
                resetForm();
                return toast.success('Device updated successfully');
            }
        } catch (error) {
            return toast.error(error?.response?.data?.message ?? error?.response?.data ?? error?.message);
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await axios({
                method: 'delete',
                url: `${BASE_URL}api/device/${selectedRowToDelete?.deviceID}`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${user?.token}`
                }
    ...
    return (
        <div>
                    ...
                    <Paper sx={{ width: '100%', overflow: 'hidden', p: 2, mb: 2 }}>
                        <Grid container direction="row" spacing={2}>
                            <Grid item lg={3} md={6} sm={6} xs={12}>
                                <Textfield name="name" label="Name (Checkme 0000)" />
                            </Grid>

                            <Grid item lg={3} md={6} sm={6} xs={12}>
                                <Textfield name="type" label="Type (Checkme)" />
                            </Grid>
            ...
            <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                <Grid container justifyContent="space-between" alignItems="center" p={2} rowGap={2}>
                            ...
                            endAdornment={
                                <InputAdornment position="end">
                                    {searchQuery?.length > 0 ? (
                                        ...
                                        <IconButton type="submit" title="Search Device" edge="end">
                                            <SearchIcon />
                                        </IconButton>
                                    )}
                                </InputAdornment>
                    ...
                    <Grid sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
                        <IconButton title="Print Device List">
                            <PrintIcon />
                        </IconButton>
                    </Grid>
                </Grid>
                    ...
                    <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody>
                            ...
                            {!!errorDeviceList && (
                                <TableRow sx={{ height: 300 }}>
                                    <TableCell align="center" colSpan={6}>
                                        <Typography>{errorDeviceList?.response?.data?.message ?? errorDeviceList?.message}</Typography>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
            ...
            <ModalCustom open={openUpdateModal} title={'Update Device'}>
                <Formik
                    initialValues={{
                        ...selectedRowToUpdate
                    }}
                    ...
                    <Form>
                        <Grid container direction="row" spacing={2}>
                            <Grid item lg={6} md={6} sm={6} xs={12}>
                                <Textfield name="name" label="Name (Checkme xxxx)" />
                            </Grid>
