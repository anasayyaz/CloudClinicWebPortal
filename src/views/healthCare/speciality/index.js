...
export default function Speciality() {
    ...
    const {
        data: specialityList,
        loading: loadingSpecialityList,
        error: errorSpecialityList,
        refetch: refetchSpecialityList
    } = useFetch(`${BASE_URL}api/colorcode`);

    const filteredSpecility = specialityList?.filter((item) => item?.speciality?.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    const FORM_VALIDATION = Yup.object().shape({
        speciality: Yup.string().trim().required('Required'),
        colorHexaCode: Yup.string().trim().required('Required')
    });
    ...
    const handleSubmit = async (values, resetForm) => {
        ...
        try {
            ...
            if (res?.data) {
                refetchSpecialityList();
                resetForm();
                return toast.success('Speciality Added Successfully');
            }
        } catch (error) {
    ...
    const handleUpdate = async (values, resetForm) => {
        ...
        try {
            ...
            const res = await axios({
                method: 'put',
                data,
                url: `${BASE_URL}api/colorcode/${selectedRowToUpdate?.id}`,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${user?.token}`
                }
            ...
            if (res?.data) {
                ...
                resetForm();
                return toast.success('Speciality updated successfully');
            }
        } catch (error) {
            return toast.error(error?.response?.data?.message ?? error?.response?.data ?? error?.message);
        } finally {
    ...
    return (
        <div>
                    ...
                    <Paper sx={{ width: '100%', overflow: 'hidden', p: 2, mb: 2 }}>
                        <Grid container direction="row" spacing={2}>
                            <Grid item lg={5} md={4.5} sm={4.5} xs={12}>
                                <Textfield name="speciality" label="Speciality" />
                            </Grid>

                            <Grid item lg={5} md={4.5} sm={4.5} xs={12}>
                                <Textfield name="colorHexaCode" label="Color Code" />
                            </Grid>
            ...
            <Paper sx={{ width: '100%', overflow: 'hidden' }}>
                <Grid container justifyContent="space-between" alignItems="center" p={2} rowGap={2}>
                            ...
                            endAdornment={
                                <InputAdornment position="end">
                                    {searchQuery?.length > 0 ? (
                                        ...
                                        <IconButton type="submit" title="Search Speciality" edge="end">
                                            <SearchIcon />
                                        </IconButton>
                                    )}
                                </InputAdornment>
                    ...
                    <Grid sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
                        <IconButton title="Print Speciality List">
                            <PrintIcon />
                        </IconButton>
                    </Grid>
                </Grid>
                    ...
                    <Table stickyHeader aria-label="sticky table">
                        <TableHead>
                            ...
                            >
                                ...
                                <TableCell align={'left'}>Color Code</TableCell>
                        ...
                        <TableBody>
                                ...
                                filteredSpecility?.map((row) => {
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell align={'left'}>{row?.colorHexaCode}</TableCell>

                                            <TableCell align={'left'}>
                                                <Box
                                                    sx={{ width: 120, height: 20, borderRadius: 1, backgroundColor: row?.colorHexaCode }}
                                                ></Box>
                                            </TableCell>
                            ...
                            {!!errorSpecialityList && (
                                <TableRow sx={{ height: 300 }}>
                                    <TableCell align="center" colSpan={6}>
                                        <Typography>
                                            {errorSpecialityList?.response?.data?.message ?? errorSpecialityList?.message}
                                        </Typography>
                                    </TableCell>
                                </TableRow>
                            )}
            ...
            <ModalConfirmation
                ...
                description={'Are you sure, you want to delete this speciality?'}
